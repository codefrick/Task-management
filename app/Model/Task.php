...
/* ✅ New function to auto-update overdue tasks */
function update_overdue_tasks($conn){
    $sql = "UPDATE tasks 
...
function get_my_tasks_due_today($conn, $id){
...
function get_my_tasks_NoDeadline($conn, $id){
	$sql = "SELECT * FROM tasks WHERE assigned_to = ? AND status != 'completed' AND (due_date IS NULL OR due_date = '0000-00-00') ORDER BY id DESC";
	$stmt = $conn->prepare($sql);
	$stmt->execute([$id]);
